...
async function main() {
  ...
  const backend = { Request, Response, URL, Uint8Array, Set, Date, Number, String, Error, atob:data=>Buffer.from(data,'base64').toString('binary'),
    ...
    fetch:async(url,options)=>{
      ...
      const body=JSON.parse(options.body);if(body.status==='cancelled'){assert(url.includes('id=eq.77777777-7777-4777-8777-777777777777'));assert(url.includes('inviter_id=eq.owner'));assert(url.includes('status=eq.pending'));}if(body.read_at){assert(url.includes('kind=eq.neighborhood_invitation'));operations.push('read');return new Response(JSON.stringify([{id:'notification-test',...body}]));}operations.push(body.status || 'family');return new Response(JSON.stringify([{...row,...body}]));
    }
  };
  ...
  operations.length=0;const cancelled=await (await handler(request({action:'cancel',id:neighborhoodRow.id,kind:'neighborhood'}))).json();assert.equal(cancelled.cancelled,true);assert(operations.includes('cancelled'));
