...
async function main() {
  ...
  const backend = { Request, Response, URL, Uint8Array, Set, Date, Number, String, Error, atob:data=>Buffer.from(data,'base64').toString('binary'),
    ...
    fetch:async(url,options)=>{
      ...
      const body=JSON.parse(options.body);if(body.status==='cancelled'){assert(url.includes('neighborhood_id=eq.55555555-5555-4555-8555-555555555555'));assert(url.includes('inviter_id=eq.owner'));assert(url.includes('email=eq.resident%40example.test'));assert(url.includes('status=eq.pending'));}if(body.read_at){assert(url.includes('recipient_id=eq.owner'));assert(url.includes('kind=eq.neighborhood_invitation'));operations.push('read');return new Response(JSON.stringify([{id:'notification-test',...body}]));}operations.push(body.status || 'family');return new Response(JSON.stringify([{...row,...body}]));
    }
  };
