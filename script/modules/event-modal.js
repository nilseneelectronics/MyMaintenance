...
(function () {
    function init() {
        ...
        function eventDateValue(field) { return pickers.dateValue(field); }
        function setEventDate(field, value) { pickers.setDate(field, value); }

        function closeEventCalendar() { pickers.closeAll(); }

        function closeEventTimePicker() { pickers.closeAll(); }
        ...
        function hasEventText() {
            const fields = [evName, evStartDate, evFinishDate, evStartTime, evFinishTime, evLocation, evDesc];
            return fields.some(function (el) { return el && String(el.value || '').trim() !== ''; });
        }

        function requestClose() {
            if (!hasEventText() || eventStateSnapshot() === initialEventState) return close();
            window.MyMaintenanceCommonUi.confirmDiscard(close);
        }

        if (evStartDate) pickers.attachDateField(evStartDate);
        if (evFinishDate) pickers.attachDateField(evFinishDate, { finishDate: true });
        if (evStartTime) pickers.attachTimeField(evStartTime);
        if (evFinishTime) pickers.attachTimeField(evFinishTime);
